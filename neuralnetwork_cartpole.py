...
def lreLu(x):
	alpha=0.2
	return tf.neuralNetwork.relu(x)-alpha*tf.neuralNetwork.relu(-x)
...
def neuralNetwork(obs,in_w,in_b,hid_w,out_w):
...
def run_env(env,in_w,in_b,hid_w,out_w):
	...
	for t in range(300):
		...
		action = neuralNetwork(obs,in_w,in_b,hid_w,out_w)
